...
public class EventAttendanceServiceImpl implements EventAttendanceService {
    ...
    public AttendanceStatusDto getAttendanceStatus(Long employeeId) {
        ...
        EventAttendanceResponseDto lastEvent = repository.findByEmployeeIdAndDate(employeeId, today)
                .stream()
    ...
    @Override
    @Transactional
    public EventAttendanceResponseDto registerAttendance(Long employeeId, String device) {
        ...
        return mapper.toDto(repository.save(attendance));
    }

    @Override
    public List<EventAttendanceResponseDto> list(LocalDate date) {
        List<EventAttendance> attendances = (date != null)
                ? repository.findByDate(date)
                : repository.findAll();
        return mapper.toList(attendances);
    }

    @Override
    public String determineEventType(Long employeeId, LocalDate date) {
        Optional<EventAttendance> lastEventOpt = repository.findByEmployeeIdAndDate(employeeId, date)
                .stream()
                .max((e1, e2) -> e1.getHour().compareTo(e2.getHour()));
    ...
    // Valida el horario y determina el estado del evento (PUNTUAL, TARDANZA, etc.)
